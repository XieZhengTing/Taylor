...
MODULE CONSTITUTE_VISCOELASTIC_MOD
    USE FINT_FUNCTIONS
    USE DETERMINANT_MOD
    USE INVERSE_MOD
    IMPLICIT NONE
    PRIVATE
    PUBLIC :: VISCO_ELASTIC

CONTAINS	  

      SUBROUTINE VISCO_ELASTIC( PROPS, DLT, FMAT, & ! IN
								LSTRESS, H_STRESS, S_STRESS, ierr) ! OUT
	  !$ACC ROUTINE SEQ
...
!	  IMPLICIT NONE
	  ...
	  RELAX_TIME = ETA/SHEARMOD
		...
		IF (J <= 1.0D-12) THEN ! Check for non-positive or very small determinant
		    ierr = 1 ! Error code for invalid determinant
		    LSTRESS = HUGE(0.0D0)
		    RETURN
		END IF
		...
		CALL INVERSE(FMAT_BAR, 3, INV_FMAT_BAR, inv_call_err_flag)
		IF (inv_call_err_flag /= 0) THEN
		    ierr = 2 ! Error code for inversion failure
		    LSTRESS = HUGE(0.0D0)
		    RETURN
		END IF
	  ...
	  RETURN
	  END SUBROUTINE
END MODULE CONSTITUTE_VISCOELASTIC_MOD