      ...
      ! an !$ACC UPDATE DEVICE(GMAXN, DIM_NN_LIST) would be needed here.
        ...
        IF (LINIT) THEN
          ...
          GMAXN=GNUMP             ! 與 OpenMP 版本一致
            ...
            IF (ALLOCATED(GSTACK_SHP)) DEALLOCATE(GSTACK_SHP)
            IF (ALLOCATED(GSTACK_DSHP)) DEALLOCATE(GSTACK_DSHP)
            IF (ALLOCATED(GSTACK_DDSHP)) DEALLOCATE(GSTACK_DDSHP)
            IF (ALLOCATED(GINVK)) DEALLOCATE(GINVK)
       ...
       !  SHOULD BE RELATED TO SMOOTHING FACTOR
        ...
        ! ADD SOME SAFE LENGTH TO THE EDGES
		 ...
		 BINS_SAFETY_FAC = 5 !NOT USED
          ...
          IF (ALLOCATED(JSPACE)) DEALLOCATE(JSPACE)
          IF (ALLOCATED(KSPACE)) DEALLOCATE(KSPACE)
          IF (ALLOCATED(NODES_IN_BIN)) DEALLOCATE(NODES_IN_BIN)
          IF (ALLOCATED(NODELIST_IN_BIN)) DEALLOCATE(NODELIST_IN_BIN)
