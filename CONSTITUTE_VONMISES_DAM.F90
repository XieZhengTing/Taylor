...
MODULE CONSTITUTE_VONMISES_DAM_MOD
    USE FINT_FUNCTIONS ! 以及 VON_MISES_DAM 可能需要的其他模組
    IMPLICIT NONE
    PRIVATE
    PUBLIC :: VON_MISES_DAM

CONTAINS	  

	  

      SUBROUTINE VON_MISES_DAM(STRESS, TOTAL_STRAIN, INC_STRAIN, STRESS_PREDICT, STATE, PROPS, ierr)
	  !$ACC ROUTINE SEQ
...
!	  IMPLICIT NONE
	  ...
	  RETURN
	  END SUBROUTINE
END MODULE CONSTITUTE_VONMISES_DAM_MOD		