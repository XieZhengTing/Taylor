...
120       CONTINUE
  ...
  IF (CTEMP.eq.'*FEM OUTPUT') THEN !*MODEL FILE CARD
     PRINT *, '=== DEBUG: Found *FEM OUTPUT card ==='
      READ(10,*) !# LFEM_OUTPUT
      READ(10,*,iostat=IERROR) IFEM_OUTPUT
     PRINT *, 'DEBUG: IFEM_OUTPUT value read =', IFEM_OUTPUT
      ...
      IF (IFEM_OUTPUT .EQ. 1) THEN
         LFEM_OUTPUT = .TRUE.
        PRINT *, 'DEBUG: LFEM_OUTPUT set to .TRUE.'
      ELSE IF (IFEM_OUTPUT .EQ. 0) THEN
         LFEM_OUTPUT = .FALSE.
        PRINT *, 'DEBUG: LFEM_OUTPUT set to .FALSE.'
      ELSE
  ...
  IF (.NOT. READ_FEM_OUTPUT) THEN
      ...
      LFEM_OUTPUT = .TRUE.  ! Changed default to TRUE
      CALL LOG_APPEND('LFEM_OUTPUT = .TRUE. (DEFAULT: FEM MESH WILL BE OUTPUT)')
      
      WRITE(*,*) ' '
      WRITE(*,*) 'LFEM_OUTPUT = .TRUE. (DEFAULT: FEM MESH WILL BE OUTPUT)'
      WRITE(*,*) ' '
  END IF
